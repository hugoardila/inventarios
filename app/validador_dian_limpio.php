...
class ValidadorDIAN {
    ...
    private function validarConfiguracionBasica($config) {
        ...
        if (!empty($config['nit_empresa']) && !preg_match('/^\d{9,15}$/', $config['nit_empresa'])) {
            $this->errores[] = "Formato de NIT inválido";
        }
        
        // Validar formato de TestSetId (UUID)
        if (!empty($config['testset_id']) && !preg_match('/^[a-f0-9]{8}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{12}$/', $config['testset_id'])) {
    ...
    private function validarEstructuraXMLFactura($factura_id, $config) {
        ...
        try {
            include 'views/ajax_facturacion_electronica.php';
    ...
    private function validarEstructuraXMLNotaCredito($nota_id, $config) {
        ...
        // Validar que el XML se pueda generar
        try {
            include 'views/ajax_notas_credito.php';
            $xml_result = generarXMLNotaCredito($nota_id, $config);
    ...
    private function validarEstructuraXMLNotaDebito($nota_id, $config) {
        ...
        // Validar que el XML se pueda generar
        try {
            include 'views/ajax_notas_debito.php';
            $xml_result = generarXMLNotaDebito($nota_id, $config);
    ...
    private function validarCamposObligatoriosDIAN($documento_id, $config) {
        ...
        if (!empty($config['resolucion_dian']) && !preg_match('/^\d{5,10}$/', $config['resolucion_dian'])) {
